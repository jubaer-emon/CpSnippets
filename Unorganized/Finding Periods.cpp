...
*/
    ...
    //smallest period string length
    ll n = a.length();
    ll k = n - pi[n-1];
    // k is the smallest POSSIBLE period
    // normally if there was no tail, (n % k == 0)
    // here the tail is added in pi[n-1] & n has the same number of extra characters (tail)
    // without that, k would still be the go to choice
    // if it don't work biggers won't work either check the tail now

    for (ll i = 0, j = k*(n/k); i < n%k; i++, j++){
        if (a[i] != a[j]){
            k = n;  //tail check failed
            break;
        }
    }
    ...
    // this is the other case where prefix equals some suffix and only use 2 times (1 time partial)
    ll z = pi[n-1];
    while (z != 0){
        ans[n-z] = true;
        z = pi[z-1];
